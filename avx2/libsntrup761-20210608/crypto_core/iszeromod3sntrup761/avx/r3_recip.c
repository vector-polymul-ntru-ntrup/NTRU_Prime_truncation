...
static
int crypto_core_from_inv3sntrup761(unsigned char *outbytes,const unsigned char *inbytes )
{
...
{
  unsigned char pre_out[p+1];
  crypto_core_from_inv3sntrup761(pre_out,inbytes);  // pre_out[761]: 0 -> has inv, -1 -> no inv
  outbytes[0] = (pre_out[p]>>7);
  return 0;
}
